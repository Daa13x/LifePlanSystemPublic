...
function seedRoadmapIfEmpty() {
  ...
  const seed = [
    ...
    { title: 'CI/CD + local installer build', detail: 'On push, GitHub Actions builds the portable bundle + Inno installer so the user can create the pipeline installer at the repo end; tagged builds publish a Release asset. Also add an in-app "Build installer" button so it can be built locally without a terminal.', resume_notes: 'STARTED: .github/workflows/build-installer.yml drafted (push to main / tag v* / manual dispatch; builds bundle, compiles Inno, uploads artifacts, attaches to Release on tags). NOT yet validated by a real Actions run — trigger it once, read logs, fix runner issues (npm ci lockfile, choco innosetup, package-portable PowerShell, Playwright Chromium download size/time). Remaining: in-app local-build button -> a POST /api/source/build-installer endpoint that runs scripts/package-portable.ps1 (npm run package:inno) with streamed status, gated + non-blocking. Local scripts already exist: npm run package:portable and npm run package:inno.', status: 'active', category: 'infra' },
