            ...
            <div className="coding-ide">
              ...
              <main className="coding-review-area">
                ...
                <details className="coding-evidence-section" open={Boolean(selectedCodingTask.toolTrace?.length)}>
                  ...
                  {(selectedCodingTask.toolTrace || []).length ? selectedCodingTask.toolTrace.map((entry) => <div className="coding-evidence-stats" key={`${entry.round}-${entry.resultHash}`}><strong>{entry.round}. {entry.name}</strong><code>{entry.path}</code>{entry.query && <span>query: {entry.query}</span>}<span>{entry.resultBytes} bytes</span><code>{entry.resultHash?.slice(0, 16)}</code></div>) : <Empty title="No tools used" body="The local model can make up to eight scoped list, literal search, or ranged file-read calls. It cannot run commands, use the network, or perform Git operations." />}
                </details>
