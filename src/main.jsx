...
function SystemStatus({ boot, planner, sessions, models, setNotice, refreshSignal }) {
  ...
  return (
    <section className="status-grid">
      ...
      <div className="panel wide-panel"><h3>Personal knowledge runtime</h3><Pill tone={runtime?.personalRetrievalEnabled ? 'good' : 'warn'}>{runtime?.personalRetrievalEnabled ? 'Enabled' : 'Checking'}</Pill><p>Build <code>{runtime?.build?.shortCommit || 'unknown'}</code> · database <code>{runtime?.activeDatabasePath || 'checking'}</code><br />{runtime ? `${runtime.coverage?.totalRetrievable || 0} eligible records: ${runtime.coverage?.counts?.activeKnowledge || 0} Knowledge, ${runtime.coverage?.counts?.activeProjects || 0} projects, ${runtime.coverage?.counts?.userChatMessages || 0} user Chat.` : 'Loading runtime diagnostics…'}<br />Last retrieval: {runtime?.lastPersonalRetrieval?.resultType || 'none'} ({runtime?.lastPersonalRetrieval?.sourceCount || 0} source(s)).</p></div>
...
function Chat({ sessions, activeSession, selectedSession, setSelectedSession, setSessions, messages, setMessages, refreshAll, setNotice, navigate, settings }) {
  ...
  const refreshCloudProviders = () => api('/api/chat/cloud-providers').then((providers) => { const connected = providers.filter((provider) => provider.configured); setCloudProviders(connected); setCloudProvider((current) => connected.some((provider) => provider.provider === current) ? current : (connected[0]?.provider || '')); setCloudModel((current) => connected.flatMap((provider) => provider.models || []).includes(current) ? current : (connected.find((provider) => provider.provider === cloudProvider)?.model || connected[0]?.model || '')); }).catch(() => {});
  useEffect(() => {
    ...
    <section className="chat-layout">
      ...
      <div className="chat-panel">
        ...
        <div className="composer">
          <div className="cloud-composer" aria-label="Cloud check controls"><span title="Cloud check"><Globe2 size={16} /></span>{cloudProviders.map((item) => <button key={item.provider} className={cx('cloud-provider-button', cloudProvider === item.provider && 'selected')} onClick={() => { setCloudProvider(item.provider); setCloudModel(item.model || ''); setCloudPreview(null); }} title={`Prepare a reviewed ${item.provider} cloud check`} aria-label={`Use ${item.provider}`}>{item.provider === 'ChatGPT' ? <Sparkles size={16} /> : item.provider.slice(0, 1)}</button>)}<button className="cloud-provider-button" onClick={() => navigate('settings')} title="Manage cloud accounts" aria-label="Manage cloud accounts"><Plus size={16} /></button>{cloudProviders.length ? <><select aria-label="Cloud provider model" value={cloudModel} onChange={(event) => { setCloudModel(event.target.value); setCloudPreview(null); }}><option value={cloudProviders.find((item) => item.provider === cloudProvider)?.model || ''}>{cloudProviders.find((item) => item.provider === cloudProvider)?.model || 'Provider default'}</option></select><select aria-label="Cloud check scope" value={cloudScope} onChange={(event) => { setCloudScope(event.target.value); setCloudPreview(null); }}><option value="latest-turn">Latest turn</option><option value="full-conversation">Full conversation</option></select><button onClick={previewCloudCheck} disabled={!cloudProvider}>Review cloud prompt</button></> : <small>No connected cloud provider. Use + to connect a browser session.</small>}</div>
