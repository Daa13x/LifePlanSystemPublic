...
try {
  ...
  assert.equal(resolveBrowserExtensionLifecycle({ heartbeatFresh: true, runningVersion: '0.1.4', expectedVersion: '0.1.4' }).lifecycleState, 'CONNECTED_CURRENT');
  assert.equal(resolveBrowserExtensionLifecycle({ heartbeatFresh: true, runningVersion: '0.1.3', expectedVersion: '0.1.4' }).lifecycleState, 'RELOAD_REQUIRED');
  const attemptedAt = new Date(Date.now() - 1000).toISOString();
  assert.equal(resolveBrowserExtensionLifecycle({ heartbeatFresh: true, runningVersion: '0.1.3', expectedVersion: '0.1.4', reloadAttempt: { expectedVersion: '0.1.4', attemptedAt, result: 'in_progress' } }).lifecycleState, 'RELOAD_IN_PROGRESS');
  assert.equal(resolveBrowserExtensionLifecycle({ heartbeatFresh: true, runningVersion: '0.1.3', expectedVersion: '0.1.4', reloadAttempt: { expectedVersion: '0.1.4', attemptedAt, result: 'in_progress' }, now: Date.now() + 20_000 }).lifecycleState, 'MANUAL_RELOAD_REQUIRED');
  assert.equal(resolveBrowserExtensionLifecycle({ heartbeatFresh: false, runningVersion: '0.1.4', expectedVersion: '0.1.4' }).lifecycleState, 'OFFLINE');
  ...
  assert.equal(policy.decide({ lifecycleState: 'RELOAD_REQUIRED', reloadRequired: true, expectedVersion: '0.1.4', reloadAttempt: null }).action, 'RELOAD');
  assert.equal(policy.decide({ lifecycleState: 'RELOAD_REQUIRED', reloadRequired: true, expectedVersion: '0.1.4', reloadAttempt: { expectedVersion: '0.1.4', attemptedAt, result: 'in_progress' } }).action, 'MANUAL', 'same-version recovery never schedules a second reload inside the bounded window');
  assert.equal(policy.decide({ lifecycleState: 'RELOAD_REQUIRED', reloadRequired: true, expectedVersion: '0.1.4', reloadAttempt: { expectedVersion: '0.1.4', attemptedAt: new Date(Date.now() - BROWSER_EXTENSION_RELOAD_WINDOW_MS - 1).toISOString(), result: 'manual_required' } }).action, 'RELOAD', 'a later recovery window may attempt once again');
