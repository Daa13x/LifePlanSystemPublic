...
const sourceDbPath = path.resolve(dbArgument);
const expectedCommit = argument('--expected-commit') || null;
assert.ok(fs.existsSync(sourceDbPath), 'The supplied protected database copy does not exist.');
const probeRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'lps-real-knowledge-runtime-'));
...
try {
  ...
  assert.equal(path.resolve(diagnostics.activeDatabasePath), dbPath, 'server uses an isolated clone of the supplied protected copy');
