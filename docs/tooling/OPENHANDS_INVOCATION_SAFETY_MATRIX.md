...
| runCli cwd containment | Prevents validation commands from running outside the repo/worktree boundary | `resolveRunCliCwd` and `scripts/verify-runcli-cwd.mjs` | `blocked` / `EBADCWD` | Fix the caller cwd; default cwd remains repo root, caller cwd is accepted only when safely contained, and escape attempts fail closed | No |
