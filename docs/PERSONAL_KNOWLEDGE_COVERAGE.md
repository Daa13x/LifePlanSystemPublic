...
Chat searches current, provenance-bearing local records before model generation. A record is eligible only when it is current and either a declarative user-authored Chat fact, explicitly reviewed into active Knowledge, or a current Workboard project. Saved user questions and requests remain transcript-only and are never evidence for a later answer. Every result retains a stable source ID, source type, timestamp and provenance. Manual **Attach Knowledge** remains separate: it deliberately adds a selected record to one conversation; it is not required for automatic personal retrieval.
...
| Saved user Chat | `chat_sessions`, `chat_messages` / Chat | Connected for declarative turns; questions and requests are transcript-only | Only non-deleted sessions and `role='user'`; history is contextual, not automatically approved memory. Interrogative and request turns are evidence-ineligible. | `chat:<id>`, conversation title and timestamp. |
